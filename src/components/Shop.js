...
const Shop = () => {
  ...
  const loadShopPosts = async () => {
    try {
      const response = await axios.get(
        `http://localhost:3333/product/get-buy-product?initial=${true}&category=shop`
      );
      if (response.status === 200) {
        setShopPosts(response.data);
      }
    } catch (error) {
      setShopPosts([]);
    }
